...
// Database
import UserModel from "../../models/Pihack_db/UserModel";
...
const customControllers = {
  ...
  */
  ...
  init: (router) => {
    const baseUrl = `${Properties.api}/user`;
    router.post(baseUrl + "/:id/changePassword", authorize(["ADMIN"]), customControllers.changePassword);
    router.get(baseUrl + "/myapi", customControllers.myapi);
  ...
  changePassword: async (req, res) => {
    try {
      const user = await UserModel.getByUsernameAndPassword(req.user.username, req.body.passwordAdmin);
      if (!user) {
